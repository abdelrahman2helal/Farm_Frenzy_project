...
void WolfIcon::onClick()
{
	...
	if (pGame->budget > 200)
	{
		...
		wolfList[count] = new Wolf(pGame, p, 70, 70, image_path);
		wolfList[count]->draw();
		pGame->animalList[pGame->animalListSize++] = wolfList[count];
		count++;
	}
}
...
void ChickIcon::onClick()
{
	...
	*/
	...
	if (pGame->budget > 100) {
		...
		pGame->animalList[pGame->animalListSize++] = chickList[count];
		count++;
		//window* pWind = pGame->getWind();
		//pWind->DrawImage(image_path, RefPoint.x, RefPoint.y, width, height);
	}
...
void CowIcon::onClick()
{
	...
	if (pGame->budget > 300 )
	{
		...
		cowList[count]->draw();
		pGame->animalList[pGame->animalListSize++] = cowList[count];
		count++;
	}
}
